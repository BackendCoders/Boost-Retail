...
const CategorySelector = () => {
  ...
  return (
    <div className="flex w-full divide-x divide-gray-300 bg-white rounded-md shadow-sm text-sm">
        ...
        {Object.keys(categories).map((cat) => (
          <p
            ...
            className={`cursor-pointer px-2 py-1 rounded hover:bg-gray-100 ${
              selectedMain === cat ? 'bg-gray-100 font-semibold text-primary-base' : 'text-gray-700'
            }`}
          >
            {cat}
          </p>
      ...
      {/* Sub Categories */}
      <div className="w-1/5 p-4 space-y-2">
        <h4 className="font-semibold text-gray-800">{selectedMain || 'Sub Categories'}</h4>
        {selectedMain &&
          Object.keys(categories[selectedMain]).map((sub) => (
            <p
              ...
              className={`cursor-pointer px-2 py-1 rounded hover:bg-gray-100 ${
                selectedSub === sub ? 'bg-gray-100 font-semibold text-primary-base' : 'text-gray-700'
              }`}
            >
              {sub}
            </p>
      ...
      {/* Types */}
      <div className="w-1/5 p-4 space-y-2">
        <h4 className="font-semibold text-gray-800">{selectedSub || 'Types'}</h4>
        {selectedSub &&
          categories[selectedMain][selectedSub].map((type) => (
            <p
              ...
              className={`cursor-pointer px-2 py-1 rounded hover:bg-gray-100 ${
                selectedType === type ? 'bg-gray-100 font-semibold text-primary-base' : 'text-gray-700'
              }`}
            >
              {type}
            </p>
        ...
        {selectedType ? (
          <>
            ...
            <div className="flex items-center gap-2 text-xs text-blue-600 cursor-pointer">
              <span onClick={handleSelectAll} className="hover:underline">Select All</span>
              <span>|</span>
              <span onClick={handleDeselectAll} className="hover:underline">Deselect All</span>
            </div>

            {/* Filtered Brand List */}
            <div className="space-y-2 max-h-[300px] overflow-auto">
              {filteredBrands.map((brand) => (
                <label key={brand} className="block text-gray-700">
                  <input
                    type="checkbox"
