...
const ConditionRow = ({ rule, onChangeField, onChangeOp, onChangeValue, onRemove }) => (
  <div className="flex gap-2 mb-2 items-center">
    ...
    />
    <button onClick={() => onRemove(rule.id)} className="text-gray-500 hover:text-red-600 text-sm font-bold">
      ✕
    </button>
  </div>
);
...
const ConditionGroup = ({ group, onChange, nested = false, onRemoveGroup }) => {
  ...
  const addRule = () => {
    const newRule = { id: uuidv4(), field: "Category", operator: "Equals", value: "" };
  ...
  return (
    <div className={`rounded-lg p-3 mt-3 ${nested ? "bg-white border border-gray-300" : "bg-gray-50 border"}`}>
      <div className="flex items-center gap-2 mb-3 relative">
        ...
        {/* + Menu */}
        <button onClick={toggleMenu} className="ml-1 text-xl font-bold text-gray-500 hover:text-black">+</button>
        {group.menuOpen && (
          <div className="absolute top-full left-16 mt-1 bg-white border border-gray-200 shadow-md rounded-sm z-10">
            <button onClick={addRule} className="flex items-center w-full px-3 py-2 text-sm hover:bg-gray-100">
              <RuleIcon /> Rule
            </button>
            <button onClick={addGroup} className="flex items-center w-full px-3 py-2 text-sm hover:bg-gray-100">
              <GroupIcon /> Group
            </button>
          </div>
        )}
        ...
        {nested && (
          <button onClick={() => onRemoveGroup(group.id)} className="ml-auto text-xl font-bold text-gray-500 hover:text-red-600">
            ✕
          </button>
        )}
      </div>
      ...
      {group.rules.map(item =>
        item.rules ? (
          <div key={item.id} className="ml-6 border-l border-dashed border-gray-400 pl-4">
            <ConditionGroup
              group={item}
              onChange={updated =>
                updateGroup({
                  rules: group.rules.map(r => (r.id === item.id ? updated : r))
                })
              }
              nested
...
const AdvancedSearch = () => {
  const [rootGroup, setRootGroup] = useState({
    ...
    rules: [
      ...
      { id: uuidv4(), field: "Title",      operator: "Equals", value: "Sales Manager" }
    ],
    menuOpen: false
  });
